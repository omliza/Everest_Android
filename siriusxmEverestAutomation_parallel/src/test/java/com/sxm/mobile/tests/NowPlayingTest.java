...
public class NowPlayingTest extends RefactorMobileTestCase {
	...
	@Test(suiteName = "NowPlaying", testName = "Now Playing - Live - Disallowed - Category", description = "Now Playing - On Demand - Disallowed - Elapsed and remaining time", enabled = true, groups = {
			"MOBANDEVER-263:EVQAAND-271" })
	public void onDemandDisallowedElapsedTime() {
		Common common = new Common(driver);
		Common.log("Validating Player controls MOBANDEVER-263");
		try {
			getPageFactory().getEvehome().clickNews();
			common.scrollUntilTextExists("News/Public Radio");
			getPageFactory().getEvehome().clickSubCatNews();
	...
	@Test(suiteName = "NowPlaying", testName = "VerifyDisallowedChannelProgressBar", description = "Now Playing - Live - Disallowed - Progress bar (no segments)", enabled = true, groups = {
			"EVQAAND-229" })
	public void VerifyDisallowedChannelProgressBar() {
		Common common = new Common(driver);
		Common.log("Validating Player controls MOBANDEVER-263");
		try {
			getPageFactory().getEvehome().clickNews();
			...
			getPageFactory().getEvehome().clickSubCatNews();
			getPageFactory().getEvehome().popChannel1();
			getPageFactory().getNowplayingpage().validateProgressBarTime();
		} catch (AndriodException ex) {
			Common.errorlog("Exception occured in : " + this.getClass().getSimpleName() + " : " + ex.getMessage());
			Assert.fail("Exception occured in : " + this.getClass().getSimpleName() + " : " + ex.getMessage());
		}
